...
function handleUserState(senderID: string, message: string) {
  console.log(
    `Handling state for user: ${senderID}, current state: ${users[senderID].state}`
  );
      ...
      userCollection
        ...
        .then((highestIdDoc) => {
          ...
          if (!users[senderID].username) {
            ...
            userCollection
              .findOne({ username: users[senderID].username })
              .then((existingUser) => {
                if (existingUser) {
            ...
            bcrypt
              ...
              .then((hash) => {
                ...
                const newUser = new User(
                  uniIdentifier,
                  users[senderID].username as string,
                  users[senderID].password as string
                );
                userCollection
                  .insertOne(newUser)
                  .then(() => {
  ...
  if (user) {
    if (loginPassword && user.userpassword) {
      ...
      if (match) {
        ...
        WA.sendMessage(
          "Incorrect password. Please try again. Enter your password:",
          senderID
        );
    ...
    WA.sendMessage(
      "Username not found. Please enter your username again:",
      senderID
    );
...
function handleAddProduct(senderID: string, message: string) {
  connectToDatabase()
    .then((db) => {
      ...
      findUserByUsername(db, users[senderID].username)
        .then((userRecord) => {
          if (!userRecord) {
            WA.sendMessage(
              "Username not found. Please enter your username again:",
          ...
          if (parts.length < 2 || isNaN(parseInt(parts[1]))) {
            WA.sendMessage(
              'Invalid format. Please use "add [productId]". For example, "add 1".',
              senderID
            );
            return;
          }
              ...
              if (product) {
                if (quantityToPurchase <= product.prodquan) {
                  addToCart(userRecord, product, quantityToPurchase).then(
                    () => {
                      const userCollection = db.collection("userCollection");
                      updateUserCart(userCollection, userRecord)
                        .then(() => {
                          WA.sendMessage(
                            `${product.prodname} has been added to your cart. Type 'view cart' to see your cart or 'checkout' to proceed to checkout.`,
                            senderID
                          );
                        })
                        .catch((err) => {
                  ...
                  WA.sendMessage(
                    `Insufficient stock for purchase, we only have ${product.prodquan} left!`,
                    senderID
                  );
                }
              } else {
                WA.sendMessage(
                  "Product not found. Please enter a valid product ID or name.",
                  senderID
                );
              }
            })
...
async function handleViewCart(senderID: string) {
  ...
  if (user.shoppingCart.length > 0) {
    ...
    cartMessage +=
      'Type "add" followed by the product ID to add more items or "checkout" to proceed.';
...
async function handleCheckout(senderID: string) {
  ...
  if (!users[senderID].username) {
    users[senderID].state = "initial";
    WA.sendMessage(
      "Please create an account or log in to proceed with checkout.",
      senderID
    );
    WA.sendMessage(
      'Please create an account, log in, or view products. (Type "create account", "login", or "view products")',
  ...
  if (user.shoppingCart.length > 0) {
    ...
    WA.sendMessage(
      `Your total is $${total}. Do you confirm the purchase? (yes/no)`,
      senderID
    );
...
function handleConfirmPurchase(senderID: string, message: string) {
  if (message === "yes") {
    ...
    WA.sendMessage(
      "Purchase canceled. You can continue to add items to your cart or proceed to checkout again.",
      senderID
    );
    users[senderID].state = "loggedIn";
    handleLoggedInActions(senderID, "");
    ...
    WA.sendMessage(
      'Please respond with "yes" or "no" to confirm your purchase.',
      senderID
    );
  }
}
